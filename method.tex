...
\begin{figure}[t!]
 ...
 \caption{Overview of SBM analyzing two raw signals standing for one week of data from two different HVACs. (1)~Decomposition of the signals in IMFs using EMD; (2)~aggregation of the IMFs based on their time scale; (3)~comparison of the partial signals (aggregated IMFs) using correlation coefficient.}
 \label{fig:diagram1}
\end{figure}

As shown in the previous section discovering the devices that are used in concert is particularly difficult.
SBM addresses this issue by decomposing each signal into a set of components, called IMF, that reveal the data structures at different time scales.
...
Consequently, SBM aggregates the IMFs that falls into the same meaningful time scales ranges (see \emph{IMF agg.} in Fig.\ref{fig:diagram1}).
...
However, comparing the IMFs obtained from different signals is not trivial.
...
These matrices are our basis to track the behavior of the devices and permit the proposed anomaly detector to identify misbehavior.
...
In fact the abnormal correlation values that could appear in the analyzed time bins are ignored by the median operator thanks to its robustness to outliers.
...
Consider the correlation matrix $C^t$ obtained with the data of the time bin $t$.
