...
SBS addresses this issue by decomposing each signal into a set of components, called IMF, that reveal the data structures at different time scales.
...
Consequently, SBS aggregates the IMFs that falls into the same meaningful time scales ranges (see \emph{IMF agg.} in Fig.\ref{fig:diagram1}).
...
These matrices are our basis to track the behavior of the devices and search for misbehavior.


\subsection{Search}\label{methodo:ano}
The Search part of the method aims at identifying misbehaving devices in an unsupervised manner.
The devices behavior is monitored via the correlation matrices presented in the previous section.
Using numerous observations SBS computes an adaptive reference that exhibits the normal devices utilization.
Then, SBS compares the computed reference with the current data and reports devices that deviate from their usual behavior.
