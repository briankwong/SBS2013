...
We now evaluate the detection performance of SBS using the traces from the Engineering Building 2 and the Cory Hall.
...
Only the data at the medium frequencies are analyzed, the other time scale ranges are discarded.
...
Eng.Bld.2& 9 (5) & 6 (5) & 1 (1) & 36 (1) & 3 (3) \\ \hline
...
However, 36 alarms are discarded as they stand for sensors errors; one GHP has missing data for the first 18 days.
...
In spite of the post-Fukushima measures to reduce the university's energy consumption, SBS reported 9 alarms corresponding to high power usages (Table \ref{tab:classif}).
...
We observed 6 alarms corresponding to abnormal low power usage, our inspection revealed that they usually correspond to energy saving initiatives from the rooms users, likely due to the electricity concerns in Japan.
...
Among these alarms 7 are classified as low power usage, however, our inspection revealed that the root causes is different than for the Engineering Building 2 dataset.
